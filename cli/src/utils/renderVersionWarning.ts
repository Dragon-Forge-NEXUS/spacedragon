...
import { getVersion } from "./getSpaceDragonVersion.js";
...
export const renderVersionWarning = (npmVersion: string) => {
  ...
  if (currentVersion.includes("beta")) {
    logger.warn("  You are using a beta version of spacedragon.");
    logger.warn("  Please report any bugs you encounter.");
  } else if (currentVersion !== npmVersion) {
    logger.warn("  You are using an outdated version of spacedragon.");
    logger.warn(
      "  Your version:",
      currentVersion + ".",
...
function checkForLatestVersion(): Promise<string> {
  return new Promise((resolve, reject) => {
    https
      .get(
        "https://registry.npmjs.org/-/package/@dragonforgecode/spacedragon/dist-tags",
        (res) => {
...
export const getNpmVersion = () =>
  ...
  checkForLatestVersion().catch(() => {
    try {
      return execSync("npm view spacedragon version").toString().trim();
    } catch {
      return null;
    }
  });
